...
export default function ProjectDetailsPage() {
  ...
  const [projectEstimations, setProjectEstimations] = useState<ProjectEstimation[]>([]);
  const [currentEstimation, setCurrentEstimation] = useState<ProjectEstimation | null>(null);
  const [showEstimationDetails, setShowEstimationDetails] = useState(false);
  const [editingEstimation, setEditingEstimation] = useState<ProjectEstimationItem | null>(null);
  ...
  useEffect(() => {
    const fetchProject = async () => {
      try {
        ...
        const activeEstimation = estimations.find(est => est.isActive) || estimations[0];
  ...
  const handleProjectUpdate = async (updates: Partial<Project>) => {
    try {
      const updatedProject = await dataManager.updateProject(project!.id, updates);
      if (updatedProject) {
  ...
  const updateEstimationItem = async (itemId: string, updates: Partial<ProjectEstimationItem>) => {
  ...
  const setActiveEstimation = async (estimationId: string) => {
    try {
      ...
      const newActive = projectEstimations.find(est => est.id === estimationId);
  ...
  const generateAgreement = async () => {
    ...
    try {
      ...
      const agreementContent = await dataManager.generateProjectAgreement(project.id, 'AG001');
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-blue-50 rounded-lg p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-blue-900">{project?.numberOfFloors}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-green-50 rounded-lg p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-green-900">{project?.projectDuration}m</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-purple-50 rounded-lg p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-purple-600">Current Estimate</p>
              <p className="text-xl font-bold text-purple-900">
                ₹{currentEstimation?.totalAmount?.toLocaleString('en-IN') || 'N/A'}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-sm font-bold text-yellow-900">
                {project?.dateCreated && new Date(project.dateCreated).toLocaleDateString('en-GB')}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between">
              ...
              <p className="text-gray-900">
                {project?.agreementDate && new Date(project.agreementDate).toLocaleDateString('en-GB')}
              </p>
            </div>
            <div className="flex justify-between">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          ...
          {projectEstimations.length > 0 && (
            <div className="mb-6">
              <h4 className="text-md font-medium text-gray-900 mb-4">Estimation Versions</h4>
              <div className="space-y-3">
                {projectEstimations.map((estimation) => (
                  <div
                    key={estimation.id}
                    className={`flex items-center justify-between p-4 border rounded-lg ${
                      estimation.isActive ? 'border-blue-500 bg-blue-50' : 'border-gray-200 bg-white'
                    }`}
                  >
                    <div>
                      <div className="flex items-center space-x-3">
                        <h5 className="font-medium text-gray-900">{estimation.name}</h5>
                        {estimation.isActive && <Badge variant="info">Active</Badge>}
                      </div>
                      <p className="text-sm text-gray-600">
                        Created: {new Date(estimation.createdDate).toLocaleDateString('en-GB')} • 
                        Updated: {new Date(estimation.updatedDate).toLocaleDateString('en-GB')} • 
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-center mb-4">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-600">{currentEstimation.items.length}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Materials Cost</p>
                  <p className="text-2xl font-bold text-green-600">
                    ₹{(currentEstimation.totalAmount * 0.7).toLocaleString('en-IN')}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Labor Cost</p>
                  <p className="text-2xl font-bold text-purple-600">
                    ₹{(currentEstimation.totalAmount * 0.3).toLocaleString('en-IN')}
                  </p>
                </div>
                <div>
    ...
    <div className="p-6 space-y-6">
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between mb-3">
                <h4 className="font-medium text-gray-900">Construction Agreement</h4>
            ...
            <div className="border border-gray-200 rounded-lg p-4 opacity-60">
              <div className="flex items-center justify-between mb-3">
                <h4 className="font-medium text-gray-900">Renovation Contract</h4>
            ...
            <div className="border border-gray-200 rounded-lg p-4 opacity-60">
              <div className="flex items-center justify-between mb-3">
                <h4 className="font-medium text-gray-900">Interior Agreement</h4>
          ...
          {/* Agreement History */}
          <div className="mt-8">
            <h4 className="text-md font-medium text-gray-900 mb-4">Agreement History</h4>
            <div className="text-center py-8 text-gray-500 border border-gray-200 rounded-lg">
              <FileText className="h-12 w-12 text-gray-400 mx-auto mb-4" />
              <p className="text-lg mb-2">No agreements generated yet</p>
              <p className="text-sm">Generate your first agreement using templates above</p>
            </div>
          </div>
        </div>
      </div>
    ...
    return (
      <DashboardLayout title="Project Not Found">
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-6">The requested project could not be found.</p>
  ...
  return (
    <>
      <DashboardLayout
        title={project.name}
        subtitle={`${project.id} • Created ${new Date(project.dateCreated).toLocaleDateString('en-GB')}`}
        headerActions={
          <div className="flex items-center space-x-3">
            <Button
              variant="outline"
      ...
      >
        <div className="space-y-6">
          <p className="text-gray-600">
            Select an estimation template to create a new versioned estimation for this project.
          </p>
          ...
          {templates.length === 0 && (
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm mt-1">Create templates in the Configurations section.</p>
            </div>
          )}
          ...
          <div className="flex justify-end space-x-4 pt-6 border-t border-gray-200">
            ...
            <Button
              onClick={() => selectedTemplate && createEstimationFromTemplate(selectedTemplate)}
